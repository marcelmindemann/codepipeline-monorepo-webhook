...
@pytest.fixture
def webhook_fixture():
  path = f"{os.path.dirname(os.path.abspath(__file__))}/webhook_fixture_structure4.json"

  os.environ['BRANCH_ROUTE'] = "false"
  os.environ["BRANCH_ROUTES"] = '["master"]'
  os.environ['PROJECT_SERVICE_MODEL'] = "full"
...
def test_get_touched_files(webhook_fixture):
  ...
  assert touched_files[0] == 'services/service1/folder1/added.py'
  assert touched_files[1] == 'services/service1/folder2/removed.py'
  assert touched_files[2] == 'services/service2/folder3/modified.py'
